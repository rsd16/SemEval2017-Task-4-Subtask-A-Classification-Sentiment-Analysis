...
from tensorflow.keras.layers import Embedding, Dense, GlobalAveragePooling1D, Conv1D, ZeroPadding1D, GaussianNoise, MaxPool2D, MaxPooling1D, Conv2D, MaxPooling2D, MaxPool1D, Reshape, Concatenate, Flatten, Input, Dropout, Bidirectional, LSTM
...
# Line 30427 in train.csv has a null value for 'text' column, sometimes. Just... sometimes.
...
for sentence in x_test['text']:
    x_test = [tokenizer_train.word_index[word] for word in sentence if word in tokenizer_train.word_index]
...
def cnn1d_model():
    print('Now the model CNN1D is being trained:')
...
def lstm_model():
    print('Now the model LSTM is being trained:')
...
def bi_lstm_model():
    print('Now the model Bi-LSTM is being trained:')
...
def cnn1d_lstm_model():
    print('Now the model CNN1D_LSTM is being trained:')
...
def creative_cnn1d_model():
    print('Now the model Creative_CNN1D is being trained:')
...
def cnn_multi_filters():
    inputs = Input(shape=(max_length,), dtype='int32')

    X_input = Embedding(input_dim=(len(tokenizer_train.word_index) + 1), output_dim=embedding_dim, weights=[embedding_matrix],
                        input_length=max_length, trainable=False)(inputs)

    X_input = GaussianNoise(0.1)(X_input)
    X_input = Dropout(0.2)(X_input)

    pool_representations = []
    kernel_sizes = [3, 4, 5]
    for kernel_size in kernel_sizes:
        feature_maps = Conv1D(filters=100, kernel_size=kernel_size, padding='valid', activation='relu')(X_input)
        pool_vectors = MaxPooling1D(2)(feature_maps)
        pool_vectors = Flatten()(pool_vectors)
        pool_representations.append(pool_vectors)

    representation = Concatenate(axis=1)(pool_representations)

    representation = Dropout(0.5)(representation)

    output = Dense(3, activation='softmax')(representation)
...
def attention_rnn():
    ...
    for i in range(layers):
        ...
        model.add(Bidirectional(LSTM(150, return_sequences=rs, recurrent_dropout=0.3)))
        model.add(Dropout(0.3))
    ...
    model.add(Dense(3, activation='softmax', activity_regularizer=l2(0.0001)))
...
models = {'Attention_RNN': attention_rnn(), 'CNN1D_Multi_Filters': cnn_multi_filters(), 'CNN1D': cnn1d_model(), 'LSTM': lstm_model(), 'Bi-LSTM': bi_lstm_model(), 'CNN1D_LSTM': cnn1d_lstm_model(), 'Creative_CNN1D': creative_cnn1d_model()}
...
for name, model in models.items():
    ...
    plt.legend(loc='best')
    plt.show()
    plt.savefig(f'deep_learning_models-{name}-accuracy.png')
    ...
    plt.legend(loc='best')
    plt.show()
    plt.savefig(f'deep_learning_models-{name}-loss.png')

    _, accuracy = model.evaluate(x_test, y_test, verbose=2)
    predictions = model.predict(x_test)
    predictions = np.argmax(predictions, axis=1)
    print(f'Accuracy for model {name} is {accuracy}')

    with open('deep_learning_models_results.txt', 'a') as file:
        file.write('{} \t {:.2f}'.format(name, accuracy * 100))
        file.write('\n')
